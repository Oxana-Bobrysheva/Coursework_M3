...
def expenses_by_category(df: pd.DataFrame, category: str, reference_date: str) -> str:
    ...
    logger.info("Starting expense analysis for category: '%s' starting from date: '%s'", category, reference_date)
    ...
    if filtered_df.empty:
        logger.warning("No expenses found for category: '%s' in the period from %s to %s",
                       category, start_date.date(), end_date.date())
    ...
    return json.dumps({'category': category, 'total_expenses': total_expenses}, ensure_ascii=False, indent=2)
